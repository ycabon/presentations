...
function currentFilterConfig(state: State) {
  return state.filterConfigs.find((config) => config.selected) as FilterConfig;
}
      ...
      function createTarget(config) {
        return {
          ...
          render(renderLabel: boolean) {
            const parameter: RangeEffectParameter | ColorEffectParameter = this
              .config;
            return renderLabel ? (
...
function renderCurrentFilters(state: State) {
  ...
  return (
    <div style="padding: 12px;">
      {effectMapping.results.map((component, index) => component.render(index))}
    </div>
  );
}
      ...
      return (
        <calcite-slider
          ...
          afterCreate={afterCreateEventHandler(
            ...
            (e, props) => {
              if (props) {
                updateFilterParameter(props.parameter, (e.target as any).value);
              }
            }
          )}
        ></calcite-slider>
