...
//>>built
define("exports ../../../../../../core/compilerUtils ./EvaluateAmbientLighting.glsl ./EvaluateAmbientOcclusion.glsl ./MainLighting.glsl ./PhysicallyBasedRendering.glsl ./PhysicallyBasedRenderingParameters.glsl ./PiUtils.glsl ../../shaderModules/BooleanPassUniform ../../shaderModules/FloatPassUniform ../../shaderModules/interfaces ../../../lighting/SceneLighting".split(" "),function(g,p,q,r,h,t,d,u,v,k,b,w){function l(e){e.constants.add("ambientBoostFactor","float",w.ambientBoost)}function m(e){e.uniforms.add(new k.FloatPassUniform("lightingGlobalFactor",
(c,a)=>a.lighting.globalFactor))}g.EvaluateSceneLighting=function(e,c){const a=e.fragment;e.include(r.EvaluateAmbientOcclusion,c);c.pbrMode!==d.PBRMode.Disabled&&e.include(t.PhysicallyBasedRendering,c);e.include(q.EvaluateAmbientLighting,c);e.include(u.PiUtils);a.code.add(b.glsl`
    const float GAMMA_SRGB = 2.1;
    const float INV_GAMMA_SRGB = 0.4761904;
    ${c.pbrMode===d.PBRMode.Disabled?"":"const vec3 GROUND_REFLECTANCE \x3d vec3(0.2);"}
...
}`);switch(c.pbrMode){case d.PBRMode.Disabled:case d.PBRMode.WaterOnIntegratedMesh:case d.PBRMode.Water:e.include(h.MainLighting);a.code.add(b.glsl`vec3 evaluateSceneLighting(vec3 normalWorld, vec3 albedo, float shadow, float ssao, vec3 additionalLight)
{
...
}`);break;case d.PBRMode.Normal:case d.PBRMode.Schematic:a.code.add(b.glsl`const float fillLightIntensity = 0.25;
...
inputs.averageAmbientRadiance = ambientLightIrradianceComponent[1] * (1.0 + GROUND_REFLECTANCE[1]);`);a.code.add(b.glsl`
        ...
        ${c.pbrMode!==d.PBRMode.Schematic||c.hasColorTexture?b.glsl`vec3 outColor = pow(blackLevelSoftCompression(outColorLinear, inputs), vec3(INV_GAMMA_SRGB));`:b.glsl`vec3 outColor = pow(max(vec3(0.0), outColorLinear - 0.005 * inputs.averageAmbientRadiance), vec3(INV_GAMMA_SRGB));`}
        return outColor;
      }
    `);break;case d.PBRMode.Simplified:case d.PBRMode.TerrainWithWater:e.include(h.MainLighting);a.code.add(b.glsl`const float roughnessTerrain = 0.5;
...
return outColor;
}`);break;default:p.neverReached(c.pbrMode);case d.PBRMode.COUNT:}};g.addAmbientBoostFactor=l;g.addLightingGlobalFactor=m;Object.defineProperty(g,Symbol.toStringTag,{value:"Module"})});