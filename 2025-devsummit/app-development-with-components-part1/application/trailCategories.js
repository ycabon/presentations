import { sqlAnd } from "./utils.js";
import { createParkWhereClause } from "./parks.js";
 ...
 */
export async function fetchTrailCategories(layer, selectedParkName) {
  await layer.load();
...
export function createTrailUseWhereClause(categories) {
  return (
    categories
      ?.map((category) =>
        Array.from(
...
const trailCategories = {
  ...
  Walking: new Set(["Foot", "Pedestrian", "Pedestrian - No Pets", "Pedetrian"]),
